...
def index():
    ...
    stats = {
        ...
        'rev_table' : df2.groupby(['Category','App']).agg({'Reviews' : 'sum',
                                                 'Rating' : 'mean'}).dropna(subset=['Rating']).sort_values('Reviews',ascending=False).reset_index().head(10).to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    ...
    index = LUpdate.dt.to_period("Y"), #parameter wajib
